	...
	node *root, double dcoeff, int direction)
{
	...
	{
		case 1 :	/* 1=UP this case is called when v has been inserted above 
					 * or skew to farEdge */
			/* do recursive calls first! */
			if (nullptr != farEdge->head->leftEdge)
				updatePair (A, nearEdge, farEdge->head->leftEdge,
					v, root, dcoeff, 1);

			if (nullptr != farEdge->head->rightEdge)
		...
		case 2 :	/* 2=DOWN called when v has been inserted below farEdge */
			if (nullptr != farEdge->tail->parentEdge)
				updatePair (A, nearEdge, farEdge->tail->parentEdge,
					v, root, dcoeff, 2);

			sib = siblingEdge (farEdge);
			if (nullptr != sib)
				updatePair (A, nearEdge, sib, v, root, dcoeff, 1);
	...
	node *newNode, double dcoeff, int direction)
{
	...
	switch (direction)
	{
		case 1 :	/* 1=UP  newNode is above the edge nearEdge */
			...
			if (nullptr != nearEdge->head->leftEdge)
				updateSubTree (A, nearEdge->head->leftEdge, v, root,
					newNode, 0.5 * dcoeff, 1);

			if (nullptr != nearEdge->head->rightEdge)
				updateSubTree (A, nearEdge->head->rightEdge, v, root,
					newNode, 0.5 * dcoeff, 1);

			updatePair (A, nearEdge, nearEdge, v, root, dcoeff, 1);
			break;

		case 2 :	/* newNode is below the edge nearEdge */
			...
			if (nullptr != sib)
				updateSubTree (A, sib, v, root, newNode, 0.5 * dcoeff, 5); // SKEW=5

			if (nullptr != nearEdge->tail->parentEdge)
				updateSubTree (A, nearEdge->tail->parentEdge, v, root,
					newNode, 0.5 * dcoeff, 2);

			updatePair (A, nearEdge, nearEdge, v, root, dcoeff, 2);
			break;

		case 5 :	/* SKEW=5 newNode is neither above nor below nearEdge */
			A[v->index][nearEdge->head->index] = A[nearEdge->head->index][v->index];
			A[newNode->index][nearEdge->head->index] =
			A[nearEdge->head->index][newNode->index] =
				0.5 * (A[nearEdge->head->index][root->index] +
...
void BMEupdateAveragesMatrix (double **A, edge *e, node *v, node *newNode)
{
	...
	if (nullptr != left)
		updateSubTree (A, left, v, e->head, newNode, 0.25, 1);		/* UP=1 updates left and below */

	if (nullptr != right)
		updateSubTree (A, right, v, e->head, newNode, 0.25, 1);	/* UP=1 updates right and below */


	sib = siblingEdge (e);
	if (nullptr != sib)
		updateSubTree (A, sib, v, e->head, newNode, 0.25, 5);	/* SKEW=5 updates sib and below */

	par = e->tail->parentEdge;
	if (nullptr != par)
		updateSubTree (A, par, v, e->head, newNode, 0.25, 2);	/* DOWN=2 updates par and above */
	...
	updatePair (A, e, e, v, e->head, 0.5, 1);	/* updates e->head fields only */
...
void BMEsplitEdge (tree *T, node *v, edge *e, double **A)
{
	...
	newNode = makeNode (T->size+1);
	newPendantEdge = makeEdge (newNode, v, 0.0);
	newInternalEdge = makeEdge (newNode, e->head, 0.0);
...
tree *BMEaddSpecies (tree *T, node *v, double **D, double **A)
{
	...
	if (nullptr == T)	//create a tree with v as only vertex, no edges
	{
		T_e = newTree();
		T_e->root = v;
...
void makeBMEAveragesTable (tree *T, double **D, double **A)
{
	...
	e = T->root->leftEdge;
	f = depthFirstTraverse (T, nullptr);
