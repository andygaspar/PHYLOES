	...
	boolean gapCheck, FILE *fpO, boolean outputMatrix)
{
	...
	if (warnCheckNegDist (D, numSeqs) && !isBoostrap)
		Warning ( (char*)"Negative distance detected. Consider the results with care.");
	if (warnCheckHighNegDist (D, numSeqs) && !isBoostrap)
		Warning ( (char*)"High value of negative distance detected. You should give up this dataset.");
...
boolean warnCheckNegDist (double **D, int numSeqs)
{
	int i, j;
	boolean ret = FALSE;
...
boolean warnCheckHighNegDist (double **D, int numSeqs)
{
	int i, j;
	boolean ret = FALSE;
